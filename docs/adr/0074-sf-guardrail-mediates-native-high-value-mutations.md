...
Mutating Anonymous Apex must never receive broad operation-family session approval. If session approval is offered outside production-like orgs, it must be exact to the verified org and normalized Apex body fingerprint; production or Unknown Org behavior should align with existing production Anonymous Apex shell-command policy.
