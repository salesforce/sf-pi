...
| `sf-docs`          | Pi settings: `sfPi.docs`; endpoint URL in Pi provider state (`credential.env.SF_DOCS_MCP_ENDPOINT`); catalog cache under `<globalAgentDir>/sf-pi/sf-docs/`.         | Collection, version, locale, fetch format, page size, display density, and catalog cache defaults. | Save in place. Explicit tool arguments win; answer/explain always request citations. The endpoint remains outside ordinary settings and is collected by `/login sf-docs`.                 |
