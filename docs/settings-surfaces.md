...
| `sf-docs`          | Pi settings: `sfPi.docs`; credential and endpoint URL in Pi auth store (`credential.env.SF_DOCS_MCP_ENDPOINT`); catalog cache under `<globalAgentDir>/sf-pi/sf-docs/`. | Collection, version, locale, fetch format, page size, citations, display density, and catalog cache defaults. | Save in place. Explicit tool arguments win; credentials and the docs endpoint remain outside ordinary settings and are collected by `/login sf-docs`.                                     |
