...
| `sf-conn/index.ts`                            | every Salesforce-org API consumer                                  | Salesforce Connection Module Interface: local status, target, version, request, and query    |
...
| `sf-conn/auth-refresh.ts`                     | `sf-conn` request and identity paths                               | One shared per-Connection authentication-refresh coordinator                                 |
| `sf-conn/request.ts`                          | `sf-conn/index.ts` implementation                                  | Bounded transport with single-flight expired-session refresh and one total deadline          |
