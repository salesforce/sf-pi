...
describe("pi version floor", () => {
  ...
  it("tracks the forward-compatible peer range and exact audited development SDK", () => {
    ...
    expect(pkg.devDependencies?.["@earendil-works/pi-coding-agent"]).toBe("0.87.1");
    expect(pkg.devDependencies?.["@earendil-works/pi-ai"]).toBe("0.87.1");
    expect(pkg.devDependencies?.["@earendil-works/pi-tui"]).toBe("0.87.1");
  });
});
...
describe("Pi compatibility policy", () => {
  it("distinguishes audited, forward-compatible, and blocked releases", () => {
    ...
    expect(RECOMMENDED_PI_VERSION).toBe("0.87.1");
    ...
    expect(isPiVersionLoadable("0.87.1")).toBe(true);
...
describe("requirePiVersion", () => {
  ...
  it("returns false and logs once when below the floor", () => {
    ...
    expect(warn.mock.calls[0][0]).toMatch(/Pi 0\.87\.1/);
  ...
  it("older Pi degrades by skipping the extension instead of throwing", () => {
    ...
    expect(warn.mock.calls[0][0]).toContain("Use Pi 0.87.1");
  });
