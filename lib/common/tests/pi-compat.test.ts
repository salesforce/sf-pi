...
describe("pi version floor", () => {
  ...
  it("tracks the forward-compatible peer range and exact audited development SDK", () => {
    ...
    expect(pkg.devDependencies?.["@earendil-works/pi-coding-agent"]).toBe("0.86.1");
    expect(pkg.devDependencies?.["@earendil-works/pi-ai"]).toBe("0.86.1");
    expect(pkg.devDependencies?.["@earendil-works/pi-tui"]).toBe("0.86.1");
  });
});
...
describe("Pi compatibility policy", () => {
  it("distinguishes audited, forward-compatible, and blocked releases", () => {
    expect(MIN_PI_VERSION).toBe("0.86.0");
    expect(AUDITED_MAX_PI_VERSION_EXCLUSIVE).toBe("0.87.0");
    expect(HARD_MAX_PI_VERSION_EXCLUSIVE).toBe("1.0.0");
    expect(RECOMMENDED_PI_VERSION).toBe("0.86.1");
    ...
    expect(classifyPiVersion("0.87.0")).toBe("forward-compatible");
    expect(classifyPiVersion("0.99.0")).toBe("forward-compatible");
    expect(classifyPiVersion("0.86.1-rc.1")).toBe("prerelease");
    ...
    expect(isPiVersionLoadable("0.86.1-rc.1")).toBe(false);
...
describe("requirePiVersion", () => {
  ...
  it("returns false and logs once when below the floor", () => {
    ...
    expect(warn.mock.calls[0][0]).toMatch(/Pi 0\.86\.1/);
  ...
  it("older Pi degrades by skipping the extension instead of throwing", () => {
    ...
    expect(warn.mock.calls[0][0]).toContain("Use Pi 0.86.1");
  });

  it("warns once across isolated module graphs for a newer stable Pi", async () => {
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const stateKey = Symbol.for("sf-pi.pi-compat.forward-warning");
    const processState = globalThis as typeof globalThis & Record<symbol, unknown>;
    delete processState[stateKey];

    const first = await import("../pi-compat.ts");
    const installed = first.getInstalledPiVersion();
    expect(installed).toBeDefined();
    expect(first.requirePiVersion(null, "sf-forward-a", "0.0.0", installed!, "1.0.0")).toBe(true);

    vi.resetModules();
    const second = await import("../pi-compat.ts");
    expect(second.requirePiVersion(null, "sf-forward-b", "0.0.0", installed!, "1.0.0")).toBe(true);
