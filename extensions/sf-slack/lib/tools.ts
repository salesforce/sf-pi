...
export function registerSlackTool(pi: ExtensionAPI): void {
  pi.registerTool<typeof SlackParams>({
    ...
    promptGuidelines: [
      ...
      "If auth fails for any Slack tool, explain that interactive credential entry is temporarily disabled and ask the user to set SLACK_USER_TOKEN before starting Pi; existing saved credentials remain usable.",
