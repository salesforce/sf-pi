...
| `agentscript_preview`   | Start/send/end live preview sessions, fetch planner traces, bulk-end sessions, and clean stale preview artifacts.                                                |
...
Use `inspect/review` before publish or after behavioral changes. It is deterministic: no hidden model call, no numeric score. Readiness is `ready`, `ready_with_warnings`, `blocked`, or `partial`. Pass `target_org` to include read-only org checks. Pass `output_path` to write a Markdown report.
