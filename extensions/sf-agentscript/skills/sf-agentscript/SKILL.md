...
Use `inspect/review` before publish or after behavioral changes. It is deterministic: no hidden model call, no numeric score. Readiness is `ready`, `ready_with_warnings`, `blocked`, or `partial`. Pass `target_org` to include read-only org checks: action-target resolution, Service Agent user readiness, and surface readiness probes such as voice/messaging channel, ServiceChannel, and published voice planner checks for channel-linked agents. Pass `output_path` to write a Markdown report.
