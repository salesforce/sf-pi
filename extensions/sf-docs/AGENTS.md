...
3. **Credential boundary.** Interactive SF Docs login collects a compatible docs endpoint URL, then uses `lib/common/secure-credential-prompt.ts` for the token. Pi alone persists and removes API-key or OAuth-compatible credentials under provider id `sf-docs`. The endpoint is stored on the credential as `env.SF_DOCS_MCP_ENDPOINT`. `SF_DOCS_MCP_TOKEN` and `SF_DOCS_MCP_ENDPOINT` remain automation fallbacks. Never write the token to project settings, examples, tests, docs, session entries, or logs. Never ship a default docs hostname.
4. **Settings are non-secret.** `lib/config-panel.ts` reports only credential/endpoint source and edits preferences. Connect may prepare `/login sf-docs` but never accepts token or endpoint input itself.
