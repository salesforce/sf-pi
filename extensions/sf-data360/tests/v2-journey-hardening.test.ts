...
describe("Data 360 v2 journey hardening", () => {
  ...
  it("returns a human-readable manifest.run report with next actions", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "pi-d360-journey-report-"));
    ...
    expect(result).toMatchObject({ ok: true, action: "manifest.run" });
    expect(result.report).toContain("Manifest run complete");
    expect(result.report).toContain("GPSProviders");
    expect(result.report).toContain("0 → 2");
    expect(result.next_actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ tool: "data360_orchestrate", action: "cleanup.plan" }),
        expect.objectContaining({ tool: "data360_query", action: "sql.verify_rows" }),
      ]),
    );
  });

  it("returns recovery context when manifest.run fails mid-journey", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "pi-d360-journey-recover-"));
    const csvPath = path.join(dir, "providers.csv");
    const manifestPath = path.join(dir, "ingest.json");
    await writeFile(csvPath, "provider_id,active\nPRV001,true\n", "utf8");
    await writeFile(
    ...
    requestMock.mockImplementation(async (request: { method: string; url: string }) => {
      ...
      if (request.url.includes("/schema") && request.method === "PUT") return { ok: true };
      if (request.url.endsWith("/ssot/data-streams") && request.method === "POST") {
        throw Object.assign(new Error("boom"), { statusCode: 500, errorCode: "INTERNAL_ERROR" });
      }
      return {};
    });
    ...
    const result = await runData360V2Action(
      {
        ...
        target_org: "AgentforceSTDM",
        allow_confirmed: true,
        params: { manifestPath, authSessionId: "test-session" },
      },
      env,
      ctx,
      undefined,
    ...
    expect(result).toMatchObject({
      ok: false,
      failedStep: "stream.create_ingest_api",
      failedDataset: "GPSProviders",
      retryable: true,
      recover_via: {
        tool: "data360_orchestrate",
        action: "manifest.run",
        params: {
          ...
          resumeFrom: "stream.create_ingest_api",
        },
      },
    });
  ...
  it("plans make_data_usable without mutating", async () => {
    const result = await runData360V2Action(
      {
        tool: "data360_orchestrate",
        action: "make_data_usable.plan",
        target_org: "AgentforceSTDM",
        params: { sourceType: "csv", targetModel: "custom DMO" },
      },
      env,
      ctx,
      undefined,
    ...
    expect(result.phases).toEqual(["connect", "prepare", "harmonize", "retrieve"]);
  ...
  it("plans Agentforce behavior investigation without mutating", async () => {
    const result = await runData360V2Action(
      {
        tool: "data360_orchestrate",
        action: "agent_behavior_investigation.plan",
        target_org: "AgentforceSTDM",
        params: { agentOrSession: "Billing Agent", timeRange: "yesterday" },
      },
      env,
      ctx,
      undefined,
    ...
    expect(result).toMatchObject({ ok: true, action: "agent_behavior_investigation.plan" });
    expect(result.steps).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ tool: "data360_observe", action: "stdm.session_timeline" }),
        expect.objectContaining({ tool: "data360_observe", action: "trace.error_traces" }),
