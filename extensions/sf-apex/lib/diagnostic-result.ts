...
import type { LspDiagnostic, LspResult } from "../../sf-lsp/lib/types.ts";
...
export type ApexDiagnosticsProvider = (
  file: string,
  cwd: string,
  timeoutMs: number,
) => Promise<LspResult>;
...
export async function diagnoseFile(
  ...
  return diagnoseApexFile(filePath, cwd, diagnose);
}

export async function diagnoseApexFile(
  filePath: string,
  cwd: string,
  diagnose: ApexDiagnosticsProvider,
): Promise<ToolResult> {
  const lspResult = await diagnose(filePath, cwd, DIAGNOSTIC_TIMEOUT_MS);
