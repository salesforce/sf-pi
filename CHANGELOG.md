...
- **Centralized Salesforce org connections behind one latest-first shared Module.** SF Apex, SOQL, Agent Script, Browser route verification, Code Analyzer ApexGuru, Data 360, Data Explorer, and environment status now resolve target orgs, authentication, API versions, bounded requests, and session refresh through `lib/common/sf-conn`. The target org's highest advertised API version wins; explicit `org-api-version` is used only when discovery fails, and missing discovery/fallback fails before the business request rather than using JSforce API 50. Data 360 raw REST callers must now pass versionless resource paths; caller-owned `/services/data/vNN.N` paths are rejected.
